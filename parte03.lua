...
-- ## [Espada]
------------------------------
function criaEspada(coluna, linha, visivel)
    local colisoes = {}
    colisoes[Constantes.PERSONAGEM] = fazColisaoDoPersonagemComAEspada
...
function fazColisaoDoPersonagemComAEspada(indice)
    table.remove(objetos, indice)
    local espada = criaEspada(0, 0, false);
    table.insert(objetos, espada)
    personagem.espada = espada
	return false
end

function fazColisaoDaEspadaComOInimigo(indiceDoInimigo)
...
function atualizaEspada()
	...
	if personagem.direcao then
		local direcao = DadosDaEspada[personagem.direcao]		
		personagem.espada.x = personagem.x + direcao.x
		personagem.espada.y = personagem.y + direcao.y
		personagem.espada.sprite = direcao.sprite
	end

    if btn(4) and personagem.direcao then
		local direcao = DadosDaEspada[personagem.direcao]
		personagem.espada.visivel = true
		personagem.espada.sprite = direcao.sprite
		personagem.espada.tempoParaDesaparecer = 15
	end		

    if personagem.espada.visivel then
        verificaColisaoComObjetos(personagem.espada, personagem.espada)
        personagem.espada.tempoParaDesaparecer = personagem.espada.tempoParaDesaparecer - 1

        if personagem.espada.tempoParaDesaparecer <= 0 then
            personagem.espada.visivel = false
        end
    end
end
-- ==============================================
...
function atualizaObjetos ()
    for indice, objeto in pairs(objetos) do
        if objeto.tipo == Constantes.ESPADA then
            atualizaEspada(objeto)
        end
...
function inicializaVariaveisDoJogo ()
    ...
    local espadaDoPersonagem = criaEspada(2,5, true)
    table.insert(objetos, espadaDoPersonagem)
...
function TIC ()
    ...
    if ESTADO_DO_JOGO == 'GANHOU' then
        cls()
        print("VC MATOU O BICHO MEMO, EM!?", 56, 40)
        print("APERTE X E VAI DE, NOVO TA?", 48, 86)	
